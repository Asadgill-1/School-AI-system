...
- **Stack:** Next.js web app (App Router), deployed by the user via GitHub to Vercel. Database, auth, and storage are **Supabase Postgres** on a dedicated new Supabase account/project created only for this platform. Connect via `DATABASE_URL` / Supabase env vars; use Supabase Auth for login and Supabase client libraries for data access. This is a Supabase-DB + Vercel-frontend split. Do not use Lovable. **Critical isolation rule:** only ever use the new, dedicated Supabase project for this app — never any other Supabase project or account. If Supabase MCP tools are available, confirm the project ref belongs to the new account before running any migration or SQL, and never touch a project you did not create for this platform.
